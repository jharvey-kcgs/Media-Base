...
| Tabletop Games | Enter or search by title ✅ done | Title, genre, players, played switch, rating | BoardGameGeek XML API2. Covers card games too (Uno, Phase 10, etc.), not just board games, per explicit request - confirmed both are real, catalogued BGG entries, not just a hopeful assumption. Scan/barcode confirmed dropped entirely - BGG's API has no barcode field at all, and any UPC-to-BGG bridge would need a third-party service of unverified reliability, the same structural risk that already got Movies' original UPC approach removed once. Genre filter uses a fixed 20-item list (the user's own trim of BGG's real ~60-category list), not a dynamic "genres in your collection" list, after weighing both approaches directly. Manufacturer and Play Time both confirmed dropped; Players is a single free-text field, not separate min/max numbers. Originally assumed free/keyless - corrected after a direct request to check: BGG's own registration guide confirms an Authorization token is required for essentially any real use, and getting one is genuinely slower than TMDb/Discogs (an approved application, which can take a week or more, not an instant self-service token) |
...
- Exact API/developer accounts to register (OMDb/TMDb, Discogs, BGG developer keys) — Movies, TV Shows, and Anime all need a free TMDb credential in lib/config.ts for their title-search auto-fill to work; Vinyl/CD needs its own separate Discogs personal token, also in lib/config.ts. Tabletop Games needs one too - originally assumed free/keyless, which was wrong; BGG requires registering an application and waiting for approval (can take a week or more, unlike TMDb/Discogs' instant self-service tokens), then generating a BGG_APPLICATION_TOKEN.
