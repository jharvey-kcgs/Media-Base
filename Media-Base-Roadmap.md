...
| Music (digital) | Enter or search by title (data layer done, screen pending) | Genre, artist, title, listened switch | Title search, MusicBrainz + Cover Art Archive (both free/keyless) - search covers both album and song titles, resolving either to the album; genre is community-tagged and often missing, so expect manual edits here more than other categories; see "where to listen" below |
...
6. **Music** - data layer done (title search, MusicBrainz + Cover Art Archive, both free/keyless - see the Where to Watch/Listen section above for why this isn't Spotify-API-based despite the original plan), screens/MusicScreen.tsx itself still pending
